...
async function snapshotSiteContent() {
  try {
    ...
    // GitHub-এ auto-push (Render env-এ token বসানো থাকলে) — সেভ করা কনটেন্ট
    // সাথে সাথেই repo-তে চলে যায়, ফলে পরের deploy/restart-এও তা ফিরে আসে।
    if (gitAutoPush.enabled()) {
      const r = await gitAutoPush.pushContent(payload);
      if (r.ok) {
        console.log('✅ GitHub auto-push সফল — হোম পেজ কনটেন্ট repo-তে স্থায়ী হয়েছে');
      } else {
        console.warn('⚠️  GitHub auto-push হয়নি: ' + r.reason);
      }
    }
    return payload;
  } catch (err) {
...
// vault থেকে কনটেন্ট restore (fresh deploy-এ DB খালি থাকলে)
